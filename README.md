...
WARNING: apt does not have a stable CLI interface. Use with caution in scripts.
```
предупредило, что не стабильно работает, собственно что и привело к трем дням ковыряния SOF в поисках ответа и решения, но видимо не достаточно моих знаний и понимания работы apt для полноценного решения данной задачи и вывода полностью всего, что отображается в терминале при установке. Строка состояничя так и не появилась, видимо она отображается в каком-то отдельном потоке.
